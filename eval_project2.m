...
if (do_trig)
  ...
  img_ref(1) = {morph_tps_wrapper(img, img, p1, cell2mat(p2(1)), 1, 0, sz)};
  img_ref(2) = {morph_tps_wrapper(img, img, p1, cell2mat(p2(2)), 1, 0, sz)};
  img_ref(3) = {morph_tps_wrapper(img, img, p1, cell2mat(p2(3)), 1, 0, sz)};
  img_ref(4) = {morph_tps_wrapper(img, img, p1, cell2mat(p2(4)), 1, 0, sz)};
  img_ref(5) = {cell2mat(img_ref(1))};
end
...
for j=1:4
  for w=0:0.1:1
    ...
    if (do_trig == 0)
      img_morphed = morph_tps_wrapper(img_source, img_dest, p_source, p_dest, w, w, sz);
    else
